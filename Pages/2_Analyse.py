...
# elif 'selected_df' in st.session_state:
#     st.session_state.df = st.session_state.selected_df
...
# def Covariance(col1, col2):
#     cov = df[[col1, col2]].cov().iloc[0, 1]
#     st.write(f"Covariance between {col1} and {col2}: {cov}")
...
# def StdDev(col_name):
#     std = df[col_name].std()
#     st.write(f"Standard Deviation of {col_name}: {std}")
...
#         col_name = st.text_input("Enter Column Name (for operations below)")

#         if st.button("Drop Column"):
#             drop_cols(col_name)

#         trim_side = st.selectbox("Trim Side", ["Left", "Right", "Both"])
#         expr = st.text_input("Character(s) to Trim")
#         if st.button("Trim Column"):
#             trim(trim_side, col_name, expr)

#         type_change = st.selectbox("Change Type To", ["str", "int", "float"])
#         if st.button("Change Column Type"):
#             ChangeTypeCol(col_name, type_change)

#     with st.expander("🔹 More Cleaning"):
...
#         fill_value = st.text_input("Value to Fill NA with")
...
#         analysis_col = st.text_input("Column for Statistics")
...
#             ConfidenceInterval(analysis_col, confidence)

#         col1 = st.text_input("Column 1 (Covariance/Correlation)")
#         col2 = st.text_input("Column 2 (Covariance/Correlation)")

#         if st.button("Covariance"):
#             Covariance(col1, col2)

#         if st.button("Correlation"):
#             Correlation(col1, col2)

#     with st.expander("🔹 Predict Values"):
#         x_col = st.text_input("Independent Variable (X)")
#         y_col = st.text_input("Dependent Variable (Y)")
#         input_val = st.number_input("Input Value for Prediction")

#         if st.button("Predict"):
#             Predict(x_col, y_col, input_val)
